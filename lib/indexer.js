...
exports.addArchive = async function (db, archive, {watch}) {
  ...
  var info = await archive.getInfo({timeout: READ_TIMEOUT})
...
async function indexArchive (db, archive) {
  ...
  try {
    ...
    var [indexMeta, archiveMeta] = await Promise.all([
      ...
      archive.getInfo({timeout: READ_TIMEOUT})
    ])
    indexMeta = indexMeta || {version: 0}
    try {
      db.emit('source-indexing', archive.url, indexMeta.version, archiveMeta.version)
    } catch (e) {
      console.error(e)
    }
...
async function readAndIndexFile (db, archive, filepath, version = false) {
  ...
  try {
    // read file
    var record = JSON.parse(await archive.readFile(filepath, {timeout: READ_TIMEOUT}))
...
async function scanArchiveForFirstIndex (db, archive) {
  var allFiles = await archive.readdir('/', {recursive: true, timeout: READ_TIMEOUT})
  var archiveInfo = await archive.getInfo({timeout: READ_TIMEOUT})
