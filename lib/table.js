...
class InjestTable extends EventEmitter {
  ...
  // (url | index, value?) => Promise<InjestArchive>
  async get (urlOrIndex, value) {
    assert(typeof urlOrIndex === 'string', ParameterError, 'Must provide a url or index to table.get()')
    if (typeof value !== 'undefined') {
      return this.where(urlOrIndex).equals(value).first()
    }
    return this.where('_url').equals(urlOrIndex).first()
  }
...
function updateByUrl (table, url, updates) {
  ...
  assert(updates && typeof updates === 'object', ParameterError, 'Invalid parameters given to update()')
  if (url.indexOf(table.name) === -1 && updates[table.schema.primaryKey]) {
