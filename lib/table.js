...
class InjestTable extends EventEmitter {
  ...
  async upsert (archive, record) {
    ...
    archive = this.db._archives[typeof archive === 'string' ? archive : archive.url]
    if (!archive) {
      throw new QueryError('Unable to upsert(): the given archive is not part of the index')
    }
    if (!archive.isWritable) {
      throw new QueryError('Unable to upsert(): the given archive is not owned by this user')
    }
