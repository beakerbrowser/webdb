...
class IngestTable extends EventEmitter {
  constructor (db, name, schema) {
    ...
    // construct db object
    this.level = IndexedLevel(db.level.sublevel(name), schema.index)
  }
  ...
  async add (archive, record, noLockNeeded = false) {
    ...
    if (this.schema.validator) {
      let isValid = this.schema.validator(record)
      if (!isValid) throw this.schema.validator.errors[0]
    }
  ...
  async delete (urlOrArchive, key) {
    if (typeof urlOrArchive === 'string') {
      return this.where('url').equals(urlOrArchive).delete()
    }
...
function getMultiByKey (table, archive, key) {
  ...
  return table.where('url').equals(`${url}/${table.name}/${key}.json`).first()
}
...
function getSingle (table, archive) {
  ...
  var url = typeof archive === 'string' ? archive : archive.url
  return table.where('url').equals(`${url}/${table.name}.json`).first()
}
...
async function updateRecord (table, record, noLockNeeded = false) {
  ...
  assert(record && typeof record.url === 'string', ParameterError, 'Invalid parameters given to update()')

  var release = noLockNeeded === true ? noop : await lock(toArchiveUrl(record.url))
  try {
    return table.where('url').equals(record.url).update(record)
  } finally {
    release()
  }
}
