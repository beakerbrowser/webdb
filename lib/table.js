...
class InjestTable extends EventEmitter {
  ...
  async upsert (archive, objOrFn) {
    ...
    assert(objOrFn && (typeof objOrFn === 'object' || typeof objOrFn === 'function'), 'The second parameter of .upsert() must be a record object or an update function')

    // update, or add
    // TODO this needs to be atomic!! (shame on me) -prf
    var changes = await this.update(archive, objOrFn)
    if (changes === 0) {
