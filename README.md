...
Abstracts over the [DatArchive API](https://beakerbrowser.com/docs/apis/dat.html) to provide a simple database-like interface in the [Beaker Browser](https://beakerbrowser.com).
...
db.schema({
  ...
  broadcasts: {
    // `primaryKey` is used to generate filenames of new records
    primaryKey: 'createdAt', // eg `/broadcasts/${record.createdAt}.json`

    // `index` sets which attrs we can query against
    index: [
      'createdAt',
...
Abstracts over the [DatArchive API](https://beakerbrowser.com/docs/apis/dat.html) to provide a simple database-like interface. Put another way, it hides the filesystem to provide a Tables and Records metaphor. Based heavily on [Dexie.js](https://github.com/dfahlander/Dexie.js) and built using IndexedDB.
